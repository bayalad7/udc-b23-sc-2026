...
function renderDialogoDetallesEvento(array $evento): void
{
    ...
    <dialog id="<?= detallesEventoDialogoId($evento) ?>" class="m-auto w-[90%] max-w-lg rounded-xl border-0 p-0 shadow-xl backdrop:bg-slate-900/50">
        <div class="max-h-[85vh] overflow-y-auto p-5">
            ...
            <?php $etiquetas = etiquetasLista($evento['etiquetas'] ?? null); ?>
            <?php if ($etiquetas !== []): ?>
            <div class="mt-3 flex flex-wrap items-center gap-1.5">
                <?= icono('etiqueta', 'h-3.5 w-3.5 shrink-0 text-slate-400') ?>
                <?php foreach ($etiquetas as $etiqueta): ?>
                <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600"><?= htmlspecialchars($etiqueta, ENT_QUOTES, 'UTF-8') ?></span>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
