...
- [x] **Panel de administración (`app/admin/`) — resuelve la pregunta abierta del Prompt 10**: mismo mecanismo de contraseña compartida que `app/asistencias` (nunca usuario/contraseña por persona), pero con su propia columna `sistema.clave_admin`, independiente de `clave_acceso`. Como ambas contraseñas se configuran por separado y en momentos distintos, las dos columnas son `NULL` por defecto — cada módulo detecta "¿ya está configurada?" con `IS NOT NULL` sobre su propia columna, nunca asumiendo que la fila `sistema` completa ya existe. `app/admin/` centraliza los reportes/estadísticas y el CRUD (alumnos, eventos, competiciones) para el staff organizador — reemplaza la idea original de `app/inscripciones/admin/` del Prompt 10 y el reporte del Prompt 11/18, que ya no se construyen aparte.
